...
async function createBooks(req, res, next) {
    ...
    try {
        ...
        const record = await book_1.BooksInstance.create({
            id,
            ...req.body,
...
async function getBooks(req, res, next) {
    try {
        ...
        const record = await book_1.BooksInstance.findAll({
            // limit,
            // offset,
            include: [
                {
...
async function getSingleBook(req, res, next) {
    try {
        const { id } = req.params;
        const record = await book_1.BooksInstance.findOne({
            where: { id },
            include: [
                {
                    model: user_1.AuthorInstance,
...
async function updateBook(req, res, next) {
    ...
    try {
        ...
        const record = await book_1.BooksInstance.findOne({ where: { id } });
        if (!record) {
